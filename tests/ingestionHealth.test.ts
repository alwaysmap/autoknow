...
// set and the stored budget settings for Manage → Sources.
...
describe('#38 ingestion health recorder', () => {
  ...
  it('reports null summary and the default budget before the first run', async () => {
    const health = await getIngestionHealth();
    expect(health.summary).toBeNull();
    expect(health.budget.dailyReingestBudgetDocs).toBe(60); // schema default
    // The claim that used to be made against a pre-computed gauge field, made instead
    // against the one function that computes it — there is no second copy to check.
    expect(budgetGauge(health.budget.dailyReingestBudgetDocs, health.budget.freeTierRequestsPerDay)
      .exceedsFreeTier).toBe(false);
  });
});

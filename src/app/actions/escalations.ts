...
export async function setEscalationStatus(formData: FormData): Promise<ActionResult> {
  return guarded(async () => {
    ...
    // PART (c) HANGS HERE: once `lib/chatPost` exists, a chat-sourced escalation posts
    // this change back to its source thread AFTER this write has committed, and never
    // before — the in-app mutation must not wait on, or be undone by, a Chat API call.
    revalidateEscalation(escalation);
  });
}

      ...
      {phases.map((p) => {
        ...
        return (
          <g key={p.id}>
            ...
            {/* The node dot is ink, so it takes the ink TOKEN — as a literal
                mid-grey it was a dark dot on dark paper, the same defect as
                CapacityChart's INK (AGENTS lesson 7). */}
            <circle cx={c.x} cy={c.y} r={5} fill="var(--fg)" />

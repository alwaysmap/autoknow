...
export function ChainSchedule({ ledger, sopMs, now, locale, onRowCard, onJump }: {
  ...
  // Grid spans from the first week of work to a little past the last work — and to
  // SOP only when SOP is close, so an over-large buffer tail doesn't donate the
  // chart to empty columns (issue #75 / the old #42). A distant SOP is marked at the
  // right edge and read in full in the buffer lane below. (Collapsing an arbitrary
  // long run of empty weeks with a marked break is the follow-up.)
  const tMin = weekFloor(Math.min(...rows.map((r) => r.startMs)));
  // Show the SOP inline unless its buffer tail would DOMINATE the chart — a tail up
  // to ~40% of the chain's own span is fine; only a runaway buffer (the old #42 case,
  // e.g. 318 days) gets the right-edge marker instead of donating empty columns.
  const tailAllowance = Math.max(28 * DAY_MS, (lastEnd - tMin) * 0.4);
  const sopInRange = sopMs != null && sopMs <= lastEnd + tailAllowance;
  const tMax = weekCeil(sopInRange ? sopMs! : Math.max(lastEnd + 14 * DAY_MS, now));
  ...
  const x = (ms: number) => labelW + ((ms - tMin) / (tMax - tMin)) * plotW;
  const colW = plotW / nWeeks;
  ...
  const bufNow = ledger.bufferDays;
  ...
  const laneStartX = laneClampX(Math.min(...rows.map((r) => r.startMs)));
  ...
  let laneMaxLevel = startBuffer ?? 0;
  {
    let level = startBuffer ?? 0;
    let px = laneStartX;
    for (const e of laneEvents) {
      const ex = laneClampX(e.atMs);
      laneFlats.push({ x1: px, x2: ex, level, projected: e.projected });
      laneRisers.push({ x: ex, from: level, to: level + e.deltaDays, kind: e.kind, projected: e.projected });
      level += e.deltaDays;
      laneMaxLevel = Math.max(laneMaxLevel, level);
      px = ex;
    }
    laneFlats.push({ x1: px, x2: x(now), level, projected: false });
  }
  const laneEndLevel = (startBuffer ?? 0) + laneEvents.reduce((s, e) => s + e.deltaDays, 0);
  ...
  return (
    <div className={styles.chartwrap}>
      <svg viewBox={`0 0 ${W} ${H}`} className={styles.scheduleSvg} role="img" aria-label={t(locale, 'clSchedule')}>
        {/* faint week gridlines — the column structure you scan down */}
        {weeks.map((wk, k) => (
        ...
        {/* SOP beyond the grid → a right-edge marker + the buffer it stands for */}
        {!sopInRange && sopMs != null && bufNow != null && bufNow > 0 && (
          <ChartLabel x={W - PAD_R} y={TOP - 16} textAnchor="end" fontSize={10} fill="var(--ok)">
            {t(locale, 'clSopBeyond', { d: bufNow })}
          </ChartLabel>
        )}

...
// container's inline size and owns its own height in rem; every existing chart here
// violates it with a `viewBox` scaled by width. For THIS chart that failure is not
// cosmetic — a fixed-aspect viewBox renders ~4px per lane at the 360px compliance width,
// where this renders 20px. So: x is a percentage of the time window (no measurement, which
...
//
// Applied through these two helpers ONLY, and to the axis ticks as well as the plot — the
...
export default function ProgramTimeline({ layout, filteredOut = 0, filteredOutHref }: ProgramTimelineProps) {
  ...
  const marks = useMemo(() => {
    const kept = layout.marks
      .filter((m) => !hidden.has(parseHealth(m.health)))
      .map((m) => ({ ...m }));
  ...
  return (
    <div className={styles.wrap}>
      ...
      {marks.length === 0 ? (
        <p className={styles.empty} role="status">{t(locale, 'timelineAllHidden')}</p>
      ) : (
        <div className={styles.plot} style={{ height: `calc(${laneCount} * ${LANE_REM}rem + 1rem)` }}>
      ...
      <div className={styles.axis} aria-hidden="true">
        {monthTicks(windowMinMs, windowMaxMs).map((ms) => (
          <span key={ms} className={styles.tick} style={{ left: at(pct(ms)) }}>

  ...
  return (
    <g>
      ...
      {/* floating needle, ringed in the surface colour so it pops off the track.
          `data-needle` / `data-track` are styling handles ONLY, so a variant can
          re-ink the dial in CSS. No geometry depends on them. */}
      <path data-needle d={needlePath(deg)} fill={color} stroke="var(--paper)" strokeWidth={1.8} strokeLinejoin="round" />
    </g>
  );
}

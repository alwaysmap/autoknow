...
/** The full reading-form text: relative inside the crossover, an absolute locale-short
 *  date past it (same shape `DateCell` renders — `month:'short', day:'numeric',
 *  year:'numeric'`), "just now" under a minute. `justNowLabel` is resolved by the
 *  caller (`t(locale, 'justNow')`) so this stays free of an i18n import. */
export function relativeTimeText(value: string | Date, now: number, locale: Locale, justNowLabel: string): string {
  const bucket = bucketRelativeTime(value, now);
  if (bucket.isAbsolute) return localDate(value, locale, { month: 'short', day: 'numeric', year: 'numeric' });

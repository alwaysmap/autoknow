...
const STRINGS = {
  ...
  clLeverMove: {
    en: "move {name}'s time here from {program} ({d} days of buffer)",
    de: 'Zeit von {name} aus {program} hierher verlagern ({d} Tage Puffer)',
    ja: '{program}（バッファ{d}日）から{name}の時間をこちらへ移す',
    ko: '{program}(버퍼 {d}일)에서 {name}의 시간을 이쪽으로 옮기기',
  },
  clLeverHandoff: {
    ...
    ja: '{from}→{to}の引き継ぎを今合意し、開始可能日に即着手できるようにする',
    ko: '{from} → {to} 인수인계를 지금 합의해 시작 가능한 날 바로 착수하게 하기',
  },
  clLeverDeclare: {

...
| [A test that synthesises the hazard a guard defends against must first prove the guard is ARMED](a-test-that-starts-page-motion-before-the-press-races-its-own-press-point.md) | writing or debugging a spec that stages a race for a client-side guard (scrolling then clicking), or a regression test for a swallowed click fails only in CI with the state untouched — indistinguishable from the bug it exists to catch |
| [A test that fails then passes on retry reports the CHECK green](a-test-that-passes-on-retry-reports-the-check-green.md) | judging from CI whether a flake fix worked, or a flake "fixed itself" with no relevant change — read the log for `Retry #1`, not the tick |
